...
import { GetPlatformData } from "@/Urql/Types/Platform/platform.types";
...
const Statics = () => {
    ...
    //Urql
    const [{ data }, refetch] = useQuery<GetIncomeStatics>({ query: GET_INCOME_STATISTIC, variables: { sellerId: id } });
    const [{ data: platform }] = useQuery<GetPlatformData>({ query: GET_PLATFORM });

    //Handler
    const getIncomes = () => {
        let upcomingIncome = data?.getIncomeStatics?.upcomingIncomes?.reduce((acc, item) => (item?.income || 0) + acc, 0) || 0;
        upcomingIncome = Math.round(upcomingIncome - (upcomingIncome * (Number(platform?.getPlatform.charge) / 100)));

        let currentIncome = data?.getIncomeStatics?.currentIncomes?.reduce((acc, item) => (item.income || 0) + acc, 0) || 0;
        currentIncome = Math.round(currentIncome - (currentIncome * (Number(platform?.getPlatform.charge) / 100)));
        ...
        <div className="border border-solid border-gray-300 rounded">
            ...
            <div className="p-5">
                ...
                <div className="mt-8 text-center">
                    <button className="bg-main py-2 px-3 text-sm text-white rounded-md w-max mx-auto" onClick={() => setOpen(true)}>Release Payment</button>
                </div>
            </div>
            <ReleasePayment
                open={open}
