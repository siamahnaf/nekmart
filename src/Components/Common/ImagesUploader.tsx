...
const ImagesUploader = ({ label, width, height, size, folderName, value, onChange, className, error = false }: Props) => {
    //State
                                ...
                                {value && value?.length > 0 && imageList.length === 0 &&
                                    value.map((item, i) => (
                                        <div className="border border-solid border-gray-200 rounded relative w-[100px]" key={i}>
                                            <Image src={process.env.NEXT_PUBLIC_IMAGE_URL + item} alt="Image" width={width} height={height} className="w-[100px] object-cover object-top rounded" key={i} priority />
                                        </div>
                                    ))
                                }
                            </div>
