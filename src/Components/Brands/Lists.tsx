...
import { GetBrandsData, DeleteBrandData } from "@/Urql/Types/Brand/brand.types";
...
const Lists = () => {
    ...
    //Urql
    const [{ data }, refetch] = useQuery<GetBrandsData>({ query: GET_BRANDS, variables: { searchInput: { limit: 20, page: pagination } }, requestPolicy: "cache-and-network" });
    const [{ data: deleteData, error, fetching }, mutate] = useMutation<DeleteBrandData>(DELETE_BRAND);
                                        ...
                                        <div className="flex gap-3 justify-center items-center">
                                            <div>
                                                <Tooltip content="Edit Brand" placement="top" className="text-xs bg-black bg-opacity-70">
                                                    <button>
                                                        <Link href={`/brands/update-brand/${slugify(item.name, { lower: true })}-${item.id}`}>
                ...
                <div className="p-3 flex gap-2 items-center">
                    ...
                    {data?.getBrands.meta.totalPages && data.getBrands.meta.totalPages > 1 &&
                        <div className="flex justify-end gap-2">
                            ...
                            <button className={`border border-black border-solid p-1.5 rounded ${pagination >= (data?.getBrands?.meta?.totalPages || 0) ? "opacity-40" : "opacity-100"}`} disabled={pagination >= (data?.getBrands?.meta?.totalPages || 0)} onClick={() => setPagination(pagination + 1)}>
